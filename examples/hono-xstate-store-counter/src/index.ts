/**
 * Hono + Actor Kit example: Counter using @xstate/store adapter.
 *
 * Demonstrates:
 * - fromXStateStore adapter for @xstate/store-based actor logic
...
app.get("/", (c) =>
  c.json({
    name: "hono-xstate-store-counter",
    description: "Counter using @xstate/store adapter",
    routes: [
      "POST /token",
